...
class Models:
    class Model:
        class Network:
            ...
            def load(self):
                ...
                with self._mutex:
                    ...
                    with open(
                        os.path.join(self._path, "options.json"), mode="r"
                    ) as options_file:
                    ...
                    self.train = udpipe2_dataset.UDPipe2Dataset.load_mappings(
                        os.path.join(self._path, "mappings.pickle")
                    )
                    self.network = udpipe2.UDPipe2(threads=self._server_args.threads)
                    self.network.construct(
                        self.args, self.train, [], [], predict_only=True
                    )
                    self.network.load(self._path, self.args.morphodita)

                    print(
                        "Loaded model {}".format(os.path.basename(self._path)),
                        file=sys.stderr,
                        flush=True,
                    )

        def __init__(
            self, names, path, network, variant, acknowledgements, server_args
        ):
            self.names = names
            ...
            if self._tokenizer is None:
                raise RuntimeError(
                    "Cannot load tokenizer from {}".format(tokenizer_path)
                )
            ...
            if (
                names[0] in server_args.preload_models
                or "all" in server_args.preload_models
            ):
                self._network.load()
        ...
        def _read(self, text, reader):
            ...
            if processing_error.occurred():
                raise RuntimeError(
                    "Cannot read input data: '{}'".format(processing_error.message)
                )
            ...
            if tag or parse:
                ...
                with self._server_args.optional_semaphore:
                    ...
                    if self._network.args.wembedding_model:
                        wembeddings = (
                            self._server_args.wembedding_server.compute_embeddings(
                                self._network.args.wembedding_model, wembedding_input
                            )
                        )
                    else:
                        wembeddings = []
                ...
                dataset = udpipe2_dataset.UDPipe2Dataset(
                    text="".join(conllu_input),
                    train=self._network.train,
                    shuffle_batches=False,
                    embeddings=wembeddings,
                    override_variant=self._variant,
                )
                ...
                if not tag:
                    network_args.tags = []
                if not parse:
                    network_args.parse = 0

                # Perform the prediction
                time_nws = time.time()
                with self._server_args.optional_semaphore:
                    time_nw = time.time()
                    predicted = self._network.network.predict(
                        dataset, evaluating=False, args=network_args
                    )
                    time_rd = time.time()
                ...
                print(
                    "Request, WE {:.2f}+{:.2f}ms,".format(
                        1000 * (time_ds - time_we), 1000 * (time_we - time_wes)
                    ),
                    "DS {:.2f}ms,".format(1000 * (time_nws - time_ds)),
                    "NW {:.2f}+{:.2f}ms,".format(
                        1000 * (time_rd - time_nw), 1000 * (time_nw - time_nws)
                    ),
    ...
    def __init__(self, server_args):
        ...
        for i in range(0, len(server_args.models), 4):
            ...
            names = [
                "-".join(parts[: None if not i else -i])
                for parts in names
        ...
        def do_GET(request):
            ...
            try:
                ...
                for name, value in urllib.parse.parse_qsl(
                    url.query, encoding="utf-8", keep_blank_values=True, errors="strict"
                ):
                    params[name] = value
            except:
            ...
            if request.command == "POST":
                if (
                    request.headers.get("Transfer-Encoding", "identity").lower()
                    != "identity"
                ):
                    return request.respond_error(
                        "Only 'identity' Transfer-Encoding of payload is supported for now."
                    )
                ...
                try:
                    ...
                    return request.respond_error(
                        "The Content-Length of payload is required."
                    )
                ...
                if url.path.startswith("/weblicht/"):
                    ...
                    try:
                        params["data"] = request.rfile.read(content_length).decode(
                            "utf-8"
                        )
                    except:
                        return request.respond_error(
                            "The payload is not in UTF-8 encoding."
                        )

                    if url.path == "/weblicht/tokenize":
                        params["tokenizer"] = ""
                    else:
                        params["input"] = "conllu"
                    params["output"] = "conllu"
                    if url.path == "/weblicht/tag":
                        params["tagger"] = ""
                    if url.path == "/weblicht/parse":
                        params["parser"] = ""
                # multipart/form-data
                    ...
                    try:
                        ...
                        for part in parser.close().get_payload():
                            ...
                            if name:
                                params[name] = part.get_payload(decode=True).decode(
                                    "utf-8"
                                )
                    except:
                        return request.respond_error(
                            "Cannot parse the multipart/form-data payload."
                        )
                # application/x-www-form-urlencoded
                    ...
                    try:
                        for name, value in urllib.parse.parse_qsl(
                            request.rfile.read(content_length).decode("utf-8"),
                            encoding="utf-8",
                        ...
                        return request.respond_error(
                            "Cannot parse the application/x-www-form-urlencoded payload."
                        )
                else:
                    return request.respond_error(
                        "Unsupported payload Content-Type '{}'.".format(
            ...
            if url.path == "/models":
                response = {
                    "models": {
                        model.names[0]: ["tokenizer", "tagger", "parser"]
                        for model in request.server._models.models_list
                    },
            ...
            # Handle /process
                ...
                if model not in request.server._models.models_by_names:
                    return request.respond_error(
                        "The requested model '{}' does not exist.".format(model)
                    )
                ...
                if "tokenizer" in params:
                    try:
                        ...
                        return request.respond_error(
                            "During tokenization, sentence longer than 1000 words was found, aborting.\nThat should only happen with presegmented input.\nPlease make sure you do not generate such long sentences.\n"
                        )
                    except:
                        return request.respond_error(
                            "An error occured during tokenization of the input."
                        )
                else:
                    try:
                        sentences = model.read(
                            params["data"], params.get("input", "conllu")
                        )
                    except TooLongError:
                        return request.respond_error(
                            "Sentence longer than 1000 words was found on input, aborting.\nPlease make sure the input sentences have at most 1000 words.\n"
                        )
                    except:
                        return request.respond_error(
                            "Cannot parse the input in '{}' format.".format(
                                params.get("input", "conllu")
                            )
                        )
                infclen = sum(
                    sum(len(word.form) for word in sentence.words[1:])
                    for sentence in sentences
                )
                ...
                # Process the data
                tag, parse, output_format = (
                    "tagger" in params,
                    "parser" in params,
                    params.get("output", "conllu"),
                )
                ...
                try:
                    for sentence in itertools.chain(sentences, ["EOF"]):
                        if (
                            sentence == "EOF"
                            or len(batch) == request.server._server_args.batch_size
                        ):
                            ...
                            if not started_responding:
                                ...
                                if weblicht:
                                    ...
                                    request.respond(
                                        "application/json",
                                        additional_headers={
                                            "X-Billing-Input-NFC-Len": str(infclen)
                                        },
                                    )
                                    request.wfile.write(
                                        json.dumps(
                            ...
                            if weblicht:
                                ...
                                request.wfile.write(
                                    json.dumps(output, ensure_ascii=False)[1:-1].encode(
                                        "utf-8"
                                    )
                                )
                            batch = []
            ...
            # Unknown URL
                ...
                request.respond_error(
                    "No handler for the given URL '{}'".format(url.path), code=404
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--concurrent", default=None, type=int, help="Concurrent computations of NN"
    )
    parser.add_argument("--logfile", default=None, type=str, help="Log path")
    parser.add_argument(
        "--max_request_size", default=4096 * 1024, type=int, help="Maximum request size"
    )
    parser.add_argument(
        "--preload_models",
        default=[],
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=str,
        help="WEmbedding models to preload",
    )
    parser.add_argument(
        "--wembedding_server",
        default=None,
        type=str,
        help="Address of an WEmbedding server",
    )
    ...
    print(
        "To stop it gracefully, either send SIGINT (Ctrl+C) or SIGUSR1.",
        file=sys.stderr,
        flush=True,
    )
    ...
    server.shutdown()
    print(
        "Stopped handling new requests, processing all current ones.",
        file=sys.stderr,
        flush=True,
    )
    server.server_close()
